...
const AddIntegrationModal = ({ isOpen, onClose, selectedDeviceId }) => {
    const [devices, setDevices] = useState([]);
    const [integrationName, setIntegrationName] = useState('');
    const [accessKey, setAccessKey] = useState('');
    const [antaresAppName, setAntaresAppName] = useState('');
    const [antaresDeviceName, setAntaresDeviceName] = useState('');
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:5000/integrations', {
    ...
    return (  
        ...
        <Modal isOpen={isOpen} onClose={onClose}>
            <h2 className="text-2xl font-bold p-4">Tambah Integrasi</h2>
